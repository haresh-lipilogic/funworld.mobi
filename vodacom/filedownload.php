...
while (($data = fgetcsv($file1, 10000, ";")) !== FALSE) {
				...
				//	$charging='ren';
					...
					$stmt1 = $conn1->prepare("INSERT INTO ".$db.".subscriber (msisdn,clickid, advid, charging_mode,subscriptionstartdate, subscriptionenddate, amount, serviceid, servicename, servicecode, token, packageid, try,txnid,chargeid,cbsent,subscriptionid,xvczaacr) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
					$stmt1->bind_param("ssssssssssssssssss",$msisdn,$clickid,$advid, $charging_mode,$date,$subscriptionenddate,$amount,$serviceid,$servicename,$servicecode,$data,$packageid,$try,$txnid,$chargeid,$cbsent,$subcriptionid,$xvczaacr);	
					$stmt1->execute();
					
					if($advid=='1149' && $serviceid==4 && $charging_mode=="act")
					{
					$kk="https://geo-tracker.smadex.com/hyperad/tracking/action/subscription?click_id=$clickid&tracker=svmobi";
					$ll=file_get_contents($kk);
					...
					$row = $result1->fetch_assoc(); 
						...
						if($charging3!='act' && $charging3!='ren' && $charging3!='trial')
						{
							...
							$stmt1 = $conn1->prepare("INSERT INTO ".$db.".subscriber (msisdn,clickid, advid, charging_mode,subscriptionstartdate, subscriptionenddate, amount, serviceid, servicename, servicecode, token, packageid, try,txnid,chargeid,cbsent,subscriptionid,xvczaacr) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
							$stmt1->bind_param("ssssssssssssssssss",$msisdn,$clickid,$advid, $charging,$date,$subscriptionenddate,$amount,$serviceid,$servicename,$servicecode,$data,$packageid,$try,$txnid,$chargeid,$cbsent,$subcriptionid,$xvczaacr);	
							$stmt1->execute();
						}
				}
