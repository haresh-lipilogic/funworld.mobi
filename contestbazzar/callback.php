...
function callback($clickid,$spil,$serviceid)
{
	...
	if ($rowcount22<1)
	{
		   ...
		   $sql = "SELECT activeuserlog.advertiserid advid,cutoff,callbackurl,nextdaycutoff,advertclickid,pubid from ".$db.".userlog activeuserlog inner join ".$db.".advertiser on activeuserlog.advertiserid = advertiser.advertiserid where clickid='".$clickid."'";
		...
		if($count>$perc)
		{
			...
			//echo "INSERT INTO ".$db.".callbackresponse(requesttime,callbackurl,callbackresponse,advertiserid,clickid,advertclickid,issent,spilower,serviceid) values($receivedate,$url1,$data,$advid,$clickid,$advertclickid,$issent,$spil,$serviceid)	";exit;
			...
			$data='';
			$receivedate =date('Y-m-d H:i:s');
			//echo "INSERT INTO ".$db.".callbackresponse(requesttime,callbackurl,callbackresponse,advertiserid,clickid,advertclickid,issent)values('$receivedate','$url1','$data','$advid','$clickid','$advertclickid','$issent')";exit;
			$stmt3 = $conn1->prepare("INSERT INTO ".$db.".callbackresponse(requesttime,callbackurl,callbackresponse,advertiserid,clickid,advertclickid,issent,spilower,serviceid) VALUES (?,?,?,?,?,?,?,?,?)");
			 $stmt3->bind_param("sssssssss",$receivedate,$url1,$data,$advid,$clickid,$advertclickid,$issent,$spil,$serviceid); 
			 $stmt3->execute();		
